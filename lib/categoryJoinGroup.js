...
categoryJoinGroup.isMember = function(uid, cid, callback) {
	db.isSortedSetMember('cid:' + cid + ':member:uids', uid, callback);
};

categoryJoinGroup.join = function(uid, cid, callback) {
	categories.getCategoryById(cid, function(err, category){
		if (!err) {
			groups.join(category.name, uid, function(err) {
				if (!err) {
					db.sortedSetAdd('cid:' + cid + ':member:uids', Date.now(), uid, callback);
				}
			});
		}
	});
	...
	categories.getCategoryById(cid, function(err, category){
		if (!err) {
			groups.leave(category.name, uid, function(err) {
				if (!err) {
					db.sortedSetRemove('cid:' + cid + ':member:uids', uid, callback);
				}
			});
		}
	});
